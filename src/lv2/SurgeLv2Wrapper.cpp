...
SurgeLv2Wrapper::SurgeLv2Wrapper(double sampleRate)
    : fSynthesizer(new SurgeSynthesizer(this)), fDataLocation(new void*[NumPorts]()),
      fOldControlValues(new float[n_total_params]()), fSampleRate(sampleRate)
{
   // needed?
...
void SurgeLv2Wrapper::handleEvent(const LV2_Atom_Event* event)
{
            ...
            (((const LV2_Atom_Object*)&event->body)->body.otype) == fUridTimePosition)
   {
      ...
      lv2_atom_object_get(obj, fUridTime_beatsPerMinute, &beatsPerMinute, fUridTime_speed, &speed,
                          fUridTime_beat, &beat, 0);
