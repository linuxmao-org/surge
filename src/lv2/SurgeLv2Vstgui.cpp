...
bool Lv2IdleRunLoop::unregisterEventHandler(VSTGUI::X11::IEventHandler *handler)
{
    ...
    auto it = std::find_if(fEvents.begin(), fEvents.end(),
        [handler](const Event &ev) -> bool { return ev.handler == handler && ev.alive; });
...
bool Lv2IdleRunLoop::unregisterTimer(VSTGUI::X11::ITimerHandler *handler)
{
    ...
    auto it = std::find_if(fTimers.begin(), fTimers.end(),
        [handler](const Timer &tm) -> bool { return tm.handler == handler && tm.alive; });
